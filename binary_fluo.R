#fluo gfp , ko =1,wt=0
anafluo <- function(prot = c(...), controle="wt",save.matsynth="n",de.matsynth="n",save.synth="n",de.synth="n",z=1,chan="chan" , fluo="fluo" , cell = c(...), data=data,sep1=c(0),text1=c(""),sep2=c(0),text2=c(""),pos.sep1=50,pos.text1=60,col.sep1=c("black"),col.text1=c("black"),pos.sep2=50,pos.text2=60,col.sep2=c("black"),col.text2=c("black"),w=9.5,h=7,
			     bottom=12,left=8.5,top=4,right=9,marge=c(bottom,left,top,right),
			     space_axis=2,space_labelY=5,space_tick=0,
                             space=c(space_labelY,space_axis,space_tick),labelY="Level IF",size_lab=3,size_main=5,size_axis=2,			     legend_size=2,pos.legend="topright",col1=c("black","turquoise2","red","gray23","mediumblue","yellow","springgreen1","magenta2","orange","lightgoldenrod4","slategray"),col2=c("turquoise1","thistle1","greenyellow"),calculation=mean) {
if (data=="choose"){
...
#par(mar=c(marge))
#par(mgp=c(space))
#matplot(l0[[1]],l0[[2]],type="p",pch=1,col=col1[seq_along(col1)%in%1:length(cell)],main=controle,xlab=prot[seq_along(prot)==1],ylab=prot[seq_along(prot)==2],cex.lab=size_lab,cex.main=size_main,cex.axis=size_axis)
#legend("topright",legend=cell,fill=col1[seq_along(col1)%in%1:length(cell)])
...
repdfwt<-c(apply(expand.grid(prot,controle), 1, paste, collapse = "_"))
...
colnames(matcalculationfin)<-as.vector(rbind(controle,cell))
...
colnames(matcalculation2)<-as.vector(rbind(controle,cell))
...
matcalculationfinwt=matcalculationfin[,grep(pattern=controle,colnames(matcalculationfin))]
matcalculationfinsample=matcalculationfin[,-grep(pattern=controle,colnames(matcalculationfin))]
matsdfinwt=matsdfin[,grep(pattern=controle,colnames(matsdfin))]
matsdfinsample=matsdfin[,-grep(pattern=controle,colnames(matsdfin))]
...
colnames(matcalculationfin2)<-c(controle,cell)
colnames(matsdfin2)<-c(controle,cell)
...
par(mar=c(marge))
par(mgp=c(space))
boxplot(matsynth112,main=i,names=c(controle,cell),las=2)
...
par(mar=c(marge))
par(mgp=c(space))
matplot(l14plot[[1]],l14plot[[2]],type="p",pch=1,col=col1[seq_along(col1)%in%1:length(c(controle,cell))],main="",xlab=prot[seq_along(prot)==1],ylab=prot[seq_along(prot)==2],cex.lab=size_lab,cex.main=size_main,cex.axis=size_axis)
legend("topright",legend=c(controle,cell),fill=col1[seq_along(col1)%in%1:length(c(controle,cell))])
}
